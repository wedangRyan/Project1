...
class Apotek_data extends CI_Model
{
    ...
    function tab_karyawan()
    {
        return $this->db->get('table_karyawan');
    }

    function tab_pasien()
    {
        return $this->db->get('table_pasien');
    }

    function tab_check()
    {
        return $this->db->get('table_check');
    }
    ...
    function get_pasien_coba($id_pasien)
    {   
        $query = "SELECT nama_pasien FROM table_pasien WHERE id_pasien='$id_pasien'";
        $hasil=$this->db->query($query);
        foreach($hasil->result() as $row) {
            $output = $row->nama_pasien;
        }
        return $output;
    }
    ...
    function get_pasien()
    {
        $data = array();
        $gabung;
        $query = $this->db->get('table_pasien')->result_array();

        if( is_array($query) && count ($query) > 0 )
        {
        foreach ($query as $row ) 
        {
          $gabung = $row['id_pasien']." | ".$row['nama_pasien'];
          $data[$row['nama_pasien']] = $gabung;
          $data[$row['id_pasien']] = $row['id_pasien'];
        }
        }
        asort($data);
        return $data;
    ...
    public function cekkodecheck()
    {
        $query = $this->db->query("SELECT MAX(id_obat) as id_obat from table_med");
        $hasil = $query->row();
    ...
    {
        $query = $this->db->query("SELECT MAX(id_check) as id_check from table_check");
        $hasil = $query->row();
        return $hasil->id_check;
    }

    public function cekkodepasien()
    {
        $query = $this->db->query("SELECT MAX(id_pasien) as id_pasien from table_pasien");
        $hasil = $query->row();
    ...
    {
        $data = array();
        $query = $this->db->get('table_check')->result_array();

        if( is_array($query) && count ($query) > 0 )
        {
        foreach ($query as $row ) 
        {
          $data[$row['id_check']] = $row['id_check'];
        }
        }
        asort($data);
        return $data;
    ...
    function get_productcheck($id_check)
    {   $hasil = array();
        $hsl=$this->db->query("SELECT * FROM table_check WHERE id_check='$id_check'");
        if($hsl->num_rows()>0){
            foreach ($hsl->result() as $data) {
                $hasil=array(
    ...
    function get_gabung($tahun_beli){
       ...
       $query = $this->db->query("SELECT m.month_name as month,
                ...
                LEFT JOIN (SELECT MONTH(tgl_beli) as month, 
                            SUM(subtotal) as total_chk  
                            FROM table_invoice
                            WHERE YEAR(tgl_beli)= '$tahun_beli' and nama_obat = '$tes'
                            GROUP BY month) t  ON (m.month_num = t.month)
                LEFT JOIN (SELECT MONTH(tgl_beli) as month, 
    ...
    function get_gabungtotal($tahun_beli){
        $tes ='-';
       $query = $this->db->query("SELECT m.month_name as month, 
                   t.total_chk,
                   i.total_inv, 
